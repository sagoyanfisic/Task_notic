...
(function ( $ ) {
    	...
    	if(color == undefined) {
			...
			$.each(array, function(date,events) {
				...
				$.each(events, function(ev,evdata) {
					...
					tempeventarray["idevento"] = evdata.idevento;
		...
		$(".calendar").each(function() {
			...
			$(this).find('[data-role=day]').each(function() {
				...
				$(this).find('[data-role=event]').each(function() {
					...
					tempeventarray["idevento"] = $(this).data("idevento");
					tempdayarray.push(tempeventarray);
				});
	...
	var tocalendar = function() {
		...
		var ev = orderBy('idevento',this.events);
		for(var o = 0; o<ev.length;o++) {
			$(".specific-day-scheme").append('<div class="specific-day-scheme-event"><h1>'+ev[o]['name']+'</h1><p data-role="dur"><a class="colorbox" href="ajax.php?accion=edicion&idevento='+ev[o]['idevento']+'"><img src="images/026.png"></a><a class="colorbox" href="ajax.php?accion=eliminar&idevento='+ev[o]['idevento']+'"><img src="images/020.png"></a></p></div>');
		}
		}
	}
	...
	function calendarSetMonth(ele,offset) {
		...
		ele.find(".calendar-month-view p").text(monthArray[d.getMonth()][1]+' '+d.getFullYear());
			...
			for(var i=0;i<42;i++) {
				...
				if (d.getDate()<10) dia1="0"+d.getDate();
				else dia1=d.getDate();
				if (hoy.getDate()<10) dia2="0"+hoy.getDate();
				else dia2=hoy.getDate();
				...
				if (mes2<10) dia2="0"+mes2;
				else mes2=mes2;
				
				var fechacal=d.getFullYear()+''+mes1+''+dia1;
				var fechahoy=hoy.getFullYear()+''+mes2+''+dia2;
				
				if (fechacal>=fechahoy)
				var cal_day = $('<div class="calendar-day"><div class="date-holder"><a class="colorbox" href="ajax.php?accion=insertar&dia='+d.getFullYear()+'-'+(d.getMonth()+1)+'-'+d.getDate()+'"><img src="images/021.png"></a>'+d.getDate()+'</div></div>');
				else
